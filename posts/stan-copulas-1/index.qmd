...
Copulas are multivariate distribution functions for random variables with uniform marginal distributions, i.e. they are functions that map the unit cube $[0,1]^d$ to $[0,1]$. They can also be described using **copula density functions** when the marginals are continuous.
...
$$
\begin{aligned}
\log h(X) &= \log c\left(u_1, \dots, u_d\right) + \sum_{i=1}^d \log f_i(X_i \vert \theta_i) \\
...
2. We need to code up some function $\log c(u_1, \dots, u_d)$ that takes as input the data $X$ after it's been transformed to $[0,1]^d$ by our CDFs and outputs a density.
